...
"""Test OP_NET token rejection (-rejecttokens)."""
...
# https://mempool.opnet.org/testnet4/tx/4d209bcefe82c836568f6d516929f2cec485106b8e118d522d0b83649cacffea
TESTNET4_WITNESS = [
...
class MempoolRejectOPNetTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 2
        self.extra_args = [
            ["-rejecttokens=1"],     # node 0: reject tokens
            ["-rejecttokens=0"],     # node 1: tokens allowed
        ]
    ...
    def test_interaction_tapscript(self):
        ...
        Interaction layout (uses OP_HASH160 for contract secret):
        https://github.com/btc-vision/opnet-node/blob/0dee2b9a/src/src/blockchain-indexer/processor/transaction/transactions/InteractionTransaction.ts#L48-L73
        """
    ...
    def test_deployment_tapscript(self):
        ...
        Deployment layout (uses OP_HASH256 for contract salt):
        https://github.com/btc-vision/opnet-node/blob/0dee2b9a/src/src/blockchain-indexer/processor/transaction/transactions/DeploymentTransaction.ts#L52-L78
        """
    ...
    def _test_opnet_witness(self, raw_tapscript):
        ...
        # Node 0 (rejecttokens=1): should reject
        result = self.nodes[0].testmempoolaccept([spend_hex])[0]
        assert_equal(result['allowed'], False)
        assert_equal(result['reject-reason'], 'bad-txns-input-tokens-op-net')
        self.log.info("  -> correctly rejected by node 0 (rejecttokens=1)")

        # Node 1 (rejecttokens=0): should NOT reject for token reasons
        ...
        self.log.info("  -> correctly allowed by node 1 (rejecttokens=0)")
